...
::: notes
The arrow \<- is read as "gets". country gets the value "Germany". Emphasize that nothing is printed when you assign — you have to call the name to see it.
:::
...
Functions can be anything from simple one-liners to complex software components. Among other things, they can be:
...
Packages are what make R -- like Python and many other programming languages -- a powerful ecosystem.
...
![](images/tidyverse-meme.png)

---

## Getting help

To open the documentation page of a function,
...
## Control Structures in R {.smaller}

| Structure | Syntax Example | Description |
|-----------|----------------|-------------|
| if        | `if (x > 0) { ... }` | Executes code if condition is TRUE |
| if-else   | `if (x > 0) { ... } else { ... }` | Chooses between two branches |
| ifelse    | `ifelse(x > 0, "pos", "neg")` | Vectorized conditional |
| for       | `for (i in 1:5) { print(i) }` | Loop over a sequence |
| while     | `while (x < 10) { x <- x + 1 }` | Repeat while condition TRUE |
| repeat    | `repeat { ...; if (x>10) break }` | Infinite loop until `break` |
| break     | `break` | Exit a loop immediately |
| next      | `next` | Skip current iteration in a loop |
...
::: notes
The native pipe |> was introduced in R 4.1. Students may also encounter %>% from the magrittr package — functionally very similar. For this course, |> is preferred.
:::
...
> Your most important collaborator is your future self — six months from now, you will not remember why you did what you did.
...
## How Is Information Organized? {.smaller}

| Structure | Dimensions | Types allowed | Formal notation | Other names (across fields) |
|---|---|---|---|----------------|
| Vector | 1 | One | $\mathbf{x}_i$ | feature vector, embedding, signal, series |
| Matrix | 2 | One | $\mathbf{X}_{i_1, i_2}$ | design matrix, adjacency matrix, image, tensor (order 2) |
| **Data frame** | 2 | **Multiple** | | dataset, table, rectangular data, tibble |
| Array | N | One | $\mathcal{X}_{i_1 i_2 \dots i_N}$ | tensor, cube, multidimensional array |
| List | Any | **Multiple** |  | container, object, record, nested data |
...
## Matrices & Arrays -- Briefly {.smaller}
...
- 2D structures storing elements of the same type  
- Fundamental for linear algebra and numerical computations  
- Data frames are more flexible for tabular data, but matrices remain crucial in statistical modeling

::: notes
Keep this brief — 2 minutes max. It's here for conceptual completeness.
:::
